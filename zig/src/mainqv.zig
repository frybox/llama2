...
//
// Override with env LLAMA2_KERNEL=scalar|avx2 (mirrors LLAMA2_KERNEL in
...
fn detectCpu(env: *std.process.Environ.Map) void {
    ...
    var flevel: u32 = 0;
    var fkernel: []const u8 = "scalar";
    var forced: bool = false;
    if (env.get("LLAMA2_KERNEL")) |e| {
        if (mem.eql(u8, e, "scalar")) {
            ...
            std.debug.print("LLAMA2_KERNEL: unknown value '{s}', ignoring\n", .{e});
        }
    }
    if (forced and flevel < level) {
        level = flevel;
        kernel = fkernel;
    }
    caps.level = level;
...
const Tokenizer = struct {
    ...
    fn parseBytePiece(piece: []const u8) ?u8 {
        if (piece.len != 6 or piece[0] != '<' or piece[1] != '0' or piece[2] != 'x' or piece[5] != '>') return null;
...
// ---------------------------------------------------------------------------
extern fn qv_matmul_avx2(o: [*]f32, wq: [*]i8, ws: [*]f32, xq: [*]i8, xs: [*]f32, n: c_int, d: c_int) callconv(.c) void;
extern fn qv_matmul_avx512(o: [*]f32, wq: [*]i8, ws: [*]f32, xq: [*]i8, xs: [*]f32, n: c_int, d: c_int) callconv(.c) void;

fn matmul(o: []f32, w: QuantizedTensor, x: QuantizedTensor) void {
    if (g_cpu.level == 2) {
        qv_matmul_avx512(o.ptr, w.q.ptr, w.s.ptr, x.q.ptr, x.s.ptr, @intCast(x.q.len), @intCast(o.len));
    } else if (g_cpu.level == 1) {
        qv_matmul_avx2(o.ptr, w.q.ptr, w.s.ptr, x.q.ptr, x.s.ptr, @intCast(x.q.len), @intCast(o.len));
    } else {
        matmul_scalar(o, w, x);
    }
}
...
pub fn main(init: std.process.Init) !void {
    ...
    const tokens_per_sec: u32 = @intFromFloat(@as(f64, @floatFromInt(pos - 1)) * std.time.ns_per_s / @as(f64, @floatFromInt(elapsed_ns)));
    log("\n\nzrunqv: total {d} tokens, speed {d} tok/s\n\n\n", .{ pos - 1, tokens_per_sec });
}
